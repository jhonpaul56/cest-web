...
function getProjectStatus(p) {
  ...
  return new Date().toISOString().split("T")[0] >= p.endDate
    ? "Completed"
    : "Active";
}
...
// ─────────────────────────────────────────────────────────
// Load ONLY real data from localStorage (no demo data)
// ─────────────────────────────────────────────────────────
function loadAllProjects() {
  try {
    return JSON.parse(localStorage.getItem("cestData") || "[]");
  } catch {
    return [];
  }
}
...
function BarChart({ projects }) {
  if (!projects.length) {
    return (
      <div style={{ color: "#aaa", textAlign: "center", paddingTop: 60, fontSize: ".9rem" }}>
        No data yet. Add projects via Data Entry.
      </div>
    );
  }
  ...
  const pad = 40;
  const gap = 8;
  const barW = Math.max(8, (W - pad * 2 - gap * (projects.length - 1)) / projects.length);
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H + 30}`} width="100%" style={{ overflow: "visible" }}>
      ...
      {projects.map((p, i) => {
        ...
        const y = pad + H - bh;
        const label = (p.community || "").slice(0, 8);
        return (
          <g key={i}>
...
function PieChart({ projects }) {
  ...
  function arc(s) {
    ...
    return `M ${cx} ${cy} L ${x1} ${y1} A ${R} ${R} 0 ${s.pct > 0.5 ? 1 : 0} 1 ${x2} ${y2} Z`;
  }
  ...
  return (
    <svg viewBox="0 0 220 175" width="100%">
      {slices.map((s, i) => <path key={i} d={arc(s)} fill={colors[i % colors.length]} opacity="0.88" />)}
      {slices.map((s, i) => {
        const mid = (s.start + s.pct / 2) * 2 * Math.PI - Math.PI / 2;
        return (
          <text key={i} x={cx + (R + 18) * Math.cos(mid)} y={cy + (R + 18) * Math.sin(mid)} fontSize="7.5" fill="#444" textAnchor="middle">
            {s.val}
          </text>
        );
      })}
      {slices.map((s, i) => (
        <g key={i} transform={`translate(178, ${12 + i * 16})`}>
          ...
          <text x="13" y="9" fontSize="7" fill="#444">{s.label.length > 22 ? s.label.slice(0, 22) + "…" : s.label}</text>
        </g>
      ))}
    </svg>
  );
...
function ViewModal({ project: p, onClose }) {
  ...
  return (
    <div onClick={(e) => e.target === e.currentTarget && onClose()} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 3000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, backdropFilter: "blur(4px)" }}>
        ...
        <div style={{ padding: 22, overflowY: "auto" }}>
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10 }}>
              {[{ label: "Male", val: male, bg: "linear-gradient(135deg,#1e5bb8,#4c84e0)" }, { label: "Female", val: female, bg: "linear-gradient(135deg,#c94ea0,#f589cf)" }, { label: "Total", val: total, bg: "linear-gradient(135deg,#6f42c1,#a66cff)" }].map(({ label, val, bg }) => (
...
function EmptyState({ onNavigate }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 20px", textAlign: "center" }}>
      ...
      <h3 style={{ color: "#fff", fontFamily: "Syne,sans-serif", fontWeight: 700, margin: "0 0 8px" }}>No Projects Yet</h3>
      <p style={{ color: "rgba(255,255,255,0.75)", marginBottom: 24, maxWidth: 360 }}>
        Your dashboard is ready. Start by adding real project data through the Data Entry page.
      </p>
      <button
        onClick={() => onNavigate("/data-entry")}
        style={{ padding: "12px 28px", borderRadius: 12, border: "none", background: "#fff", color: "#1e5bb8", fontWeight: 700, fontSize: ".95rem", cursor: "pointer", boxShadow: "0 4px 14px rgba(0,0,0,0.2)" }}
      >
        ✏️ Go to Data Entry
      </button>
    </div>
...
export default function CESTDashboard() {
  ...
  const [filtered, setFiltered] = useState(() => loadAllProjects());
  ...
  useEffect(() => {
    function refreshProjects() {
      ...
      setCategory("");
      setShowCharts(true);
      setTableLabel("Project List");
    }
    window.addEventListener("cestDataUpdated", refreshProjects);
    window.addEventListener("storage", (e) => { if (e.key === "cestData") refreshProjects(); });
    return () => {
      window.removeEventListener("cestDataUpdated", refreshProjects);
  ...
  useEffect(() => {
    function handler(e) {
      ...
      if (!e.target.closest(".search-wrap") && !e.target.closest(".cat-wrap")) { setSearchDDOpen(false); setCatDDOpen(false); }
    }
  ...
  function onSearch(val) {
    setSearch(val); setCategory("");
    if (!val) { setFiltered(projects); setShowCharts(true); setTableLabel("Project List"); setSearchDDOpen(false); return; }
    const q = val.toLowerCase();
    const f = projects.filter((p) => (p.community || "").toLowerCase().includes(q) || (p.focal || "").toLowerCase().includes(q) || (p.category || "").toLowerCase().includes(q) || (p.address || "").toLowerCase().includes(q));
    setFiltered(f); setShowCharts(false); setTableLabel(`Search: "${val}"`); setSearchDDOpen(true);
  }

  function onCategory(val) {
    setCategory(val); setSearch("");
    ...
    setFiltered(f); setShowCharts(false); setTableLabel(`Category: ${val}`); setCatDDOpen(true);
  }

  function selectResult(p) {
    setSearchDDOpen(false); setCatDDOpen(false);
    setSearch(p.community); setFiltered([p]);
    setShowCharts(false); setTableLabel(`Showing: ${p.community}`);
  }
  ...
  const ddFiltered = search ? projects.filter((p) => (p.community || "").toLowerCase().includes(search.toLowerCase()) || (p.focal || "").toLowerCase().includes(search.toLowerCase())) : [];
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#4c84e0", fontFamily: "'DM Sans', 'Segoe UI', sans-serif", overflow: "hidden" }}>
      ...
      {/* ── Sidebar ── */}
      <nav style={{ width: 250, background: "#fff", color: "#1e5bb8", position: "fixed", top: 0, left: 0, height: "100vh", zIndex: 1060, borderRight: "1px solid #e9ecef", boxShadow: "4px 0 20px rgba(0,0,0,0.08)", display: "flex", flexDirection: "column", overflowY: "auto" }}>
        ...
        <ul style={{ listStyle: "none", padding: "16px 15px", margin: 0, flex: 1 }}>
          {navLinks.map(({ icon, label, path }) => {
            ...
            return (
              <li key={label}>
                <a onClick={() => navigate(path)} style={{ color: "#1e5bb8", display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderRadius: 12, marginBottom: 6, transition: "all .3s ease", fontWeight: isActive ? 600 : 500, background: isActive ? "linear-gradient(90deg,#cce0ff,#b3d1ff)" : "none", textDecoration: "none", position: "relative", cursor: "pointer" }}>
        ...
        <div style={{ padding: "24px 32px 16px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 12, marginLeft: "auto" }}>
            <div className="bell-area" style={{ position: "relative" }}>
              <button onClick={() => setBellOpen(!bellOpen)} style={{ width: 42, height: 42, borderRadius: 12, border: "none", background: "rgba(255,255,255,0.15)", color: "white", fontSize: "1.2rem", backdropFilter: "blur(10px)", cursor: "pointer", position: "relative" }}>
                🔔
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, marginBottom: 24 }}>
                {statCards.map(({ label, val, icon, bg }) => (
                  <div key={label} style={{ borderRadius: 18, background: bg, color: "white", padding: 16, boxShadow: "0 6px 18px rgba(0,0,0,0.18)" }}>
              ...
              <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
                <div className="search-wrap" style={{ flex: 1, minWidth: 220, position: "relative" }}>
                  <input value={search} onChange={(e) => onSearch(e.target.value)} onFocus={() => search && setSearchDDOpen(true)} placeholder="Search by project, focal person, or address..." autoComplete="off" style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "none", fontSize: ".9rem", boxSizing: "border-box", outline: "none" }} />
                  {searchDDOpen && (
                    <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, right: 0, background: "#fff", borderRadius: 14, boxShadow: "0 12px 36px rgba(0,0,0,0.22)", zIndex: 600, maxHeight: 380, overflowY: "auto" }}>
                      {ddFiltered.length === 0 ? <div style={{ padding: 16, textAlign: "center", color: "#999", fontSize: ".85rem" }}>Walang resulta para sa "{search}"</div> : (
                        <>
                    ...
                    <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, right: 0, background: "#fff", borderRadius: 14, boxShadow: "0 12px 36px rgba(0,0,0,0.22)", zIndex: 600, maxHeight: 380, overflowY: "auto" }}>
                      ...
                      {catFiltered.map((p, i) => (
                        <div key={i} onClick={() => selectResult(p)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 14px", cursor: "pointer", borderBottom: "1px solid #f0f4ff" }}>
                          ...
                          <div style={{ textAlign: "right" }}><div style={{ fontSize: ".85rem", fontWeight: 700, color: "#28a745" }}>{fmt(p.budget)}</div></div>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {showCharts && (
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16, marginBottom: 24 }}>
                  <div style={{ background: "#fff", borderRadius: 15, padding: 20, boxShadow: "0 6px 18px rgba(0,0,0,0.12)" }}>
                    <h6 style={{ fontWeight: 600, marginBottom: 12, color: "#1e3a6e" }}>Budget by Community (Bar)</h6>
              ...
              <div style={{ background: "#fff", borderRadius: 15, padding: 20, boxShadow: "0 6px 18px rgba(0,0,0,0.12)", marginBottom: 24 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                  <h6 style={{ fontWeight: 600, color: "#1e3a6e", margin: 0 }}>{tableLabel}</h6>
                  <span style={{ fontSize: ".78rem", color: "#6c757d", background: "#f4f7ff", padding: "4px 12px", borderRadius: 20 }}>{projects.length} total entries</span>
                </div>
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: ".87rem" }}>
                    ...
                    <tbody>
                      {filtered.length === 0 ? (
                        <tr><td colSpan={5} style={{ textAlign: "center", color: "#aaa", padding: 24 }}>No results found.</td></tr>
                      ) : (
                        filtered.map((p, i) => {
                          const s = getProjectStatus(p);
                          return (
                            <tr key={i} style={{ borderBottom: "1px solid #f0f4ff", cursor: "pointer" }}>
