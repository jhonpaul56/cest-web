...
export default function CESTDataEntry({ onSave }) {
  ...
  const [form, setForm] = useState({
    community: "", focal: "", contact: "", address: "",
    date: "", duration: "", budget: "", households: "",
    male: "", female: "", total: "",
  });
  ...
  function saveEntry() {
    ...
    const category = checkedComponents.length > 0
      ? (CATEGORY_MAPPING[checkedComponents[0]] || checkedComponents[0])
      : "-";
      ...
      <main style={{ background: "#0c52c4", minHeight: "100vh", paddingBottom: 40, marginLeft: 250, flex: 1, overflowX: "hidden" }}>
        ...
        <div style={{ padding: "0 32px" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 18, padding: 32, boxShadow: "0 6px 18px rgba(0,0,0,0.12)", marginBottom: 32 }}>
            ...
            {/* Section 1 */}
            <h5 style={{ fontWeight: 700, color: "#1e3a6e", marginBottom: 20, fontFamily: "Syne,sans-serif", fontSize: "1rem" }}>
              🏘️ LGU / Barangay / Community / Group / Association
            </h5>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 24 }}>
              {COMPONENTS.map(({ id, value }) => {
                ...
                return (
                  <label key={id} onClick={() => toggleComponent(value)}
                    style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 10, cursor: "pointer", background: checked ? "#e8f0ff" : "#f4f7ff", border: checked ? "1.5px solid #1e5bb8" : "1.5px solid transparent", transition: "all .18s", userSelect: "none" }}>
              ...
              {[["Male", "male", "💙"], ["Female", "female", "💗"], ["Total", "total", "💜"]].map(([lbl, key, icon]) => (
                <div key={key} style={{ background: "#f4f7ff", borderRadius: 12, padding: 16, textAlign: "center" }}>
                  ...
                  <input type="number" value={form[key]} onChange={(e) => set(key, e.target.value)}
                    style={{ width: "100%", padding: "10px", borderRadius: 8, border: "1.5px solid #dde4f5", fontSize: "1rem", fontWeight: 700, color: "#1e3a6e", background: "#fff", textAlign: "center", outline: "none", boxSizing: "border-box" }}
                    placeholder="0" />
                </div>
              ))}
            </div>
            ...
            <div style={{ textAlign: "center" }}>
              <button onClick={saveEntry}
                style={{ padding: "14px 48px", borderRadius: 12, border: "none", background: "linear-gradient(135deg,#1e5bb8,#4c84e0)", color: "white", fontWeight: 700, fontSize: "1rem", cursor: "pointer", boxShadow: "0 4px 14px rgba(30,91,184,0.4)", transition: "all .2s ease", fontFamily: "Syne,sans-serif" }}>
                💾 Save Entry
              </button>
            </div>
