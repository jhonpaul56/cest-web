        ...
        <div style={{ display: "flex", borderBottom: "1px solid #eee", background: "#f8faff" }}>
          {["profile", "preferences", "notifications"].map((t) => (
            <button key={t} onClick={() => setActiveTab(t)}
              style={{ flex: 1, padding: "12px 8px", border: "none", background: activeTab === t ? "#fff" : "none", fontWeight: activeTab === t ? 700 : 500, color: activeTab === t ? "#1e5bb8" : "#6c757d", borderBottom: activeTab === t ? "2px solid #1e5bb8" : "2px solid transparent", cursor: "pointer", fontSize: ".82rem", textTransform: "capitalize" }}>
              {t}
            </button>
          ))}
        </div>
        <div style={{ padding: 22 }}>
          {activeTab === "profile" && (
            <div>
              ...
              {alert && (
                <div style={{ padding: "9px 13px", borderRadius: 9, fontSize: ".83rem", marginBottom: 12, background: alert.type === "success" ? "rgba(40,167,69,0.12)" : "rgba(220,53,69,0.1)", border: `1px solid ${alert.type === "success" ? "rgba(40,167,69,0.3)" : "rgba(220,53,69,0.28)"}`, color: alert.type === "success" ? "#1a6b2e" : "#b02020" }}>{alert.msg}</div>
              )}
              {!editing ? (
                <div>
                  ...
                  <button onClick={() => { setEditForm({ ...profile }); setEditing(true); }}
                    style={{ padding: "8px 18px", border: "none", borderRadius: 9, background: "linear-gradient(135deg,#1e5bb8,#4c84e0)", color: "white", fontSize: ".83rem", fontWeight: 600, cursor: "pointer", marginTop: 14 }}>
                    ✏️ Edit Profile
                  </button>
                </div>
              ) : (
                  ...
                  {[["Full Name", "fullname"], ["Username", "username"], ["Email Address", "email"]].map(([lbl, key]) => (
                    <div key={key} style={{ marginBottom: 12 }}>
                      ...
                      <input value={editForm[key] || ""} onChange={(e) => setEditForm({ ...editForm, [key]: e.target.value })}
                        style={{ width: "100%", padding: "10px 12px", border: "1.5px solid #dde4f5", borderRadius: 10, fontSize: ".9rem", color: "#1e3a6e", background: "#f8faff", outline: "none", boxSizing: "border-box" }} />
                    </div>
                  ))}
            ...
            <div>
              ...
              {[
                ...
                <div key={key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", borderRadius: 10, background: "#f4f7ff", marginBottom: 10 }}>
                  ...
                  <div onClick={() => setNotifs({ ...notifs, [key]: !notifs[key] })}
...
export default function CESTDocuments() {
  ...
  const [data] = useState(() => {
    try { return JSON.parse(localStorage.getItem("cestData")) || INITIAL_DATA; }
  ...
  const thStyle = (key) => ({
    ...
    fontSize: ".78rem",
    whiteSpace: "nowrap",
    background: hoveredCol === key && key !== "#" ? "#1547a0" : "transparent",
    ...
    style={{ width: 200, height: 100, objectFit: "contain", marginBottom: 8 }}/>
      ...
      <main style={{ background: "#0c52c4", minHeight: "100vh", paddingBottom: 40, marginLeft: 250, flex: 1, overflowX: "hidden" }}>
        ...
        <div style={{ padding: "24px 32px 16px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
          <h2 style={{ color: "#fff", fontWeight: 700, margin: 0, fontFamily: "Syne,sans-serif", fontSize: "1.8rem" }}>Documents</h2>
          <div style={{ display: "flex", gap: 12, marginLeft: "auto" }}>
            ...
            <button onClick={() => setShowSettings(true)}
              style={{ width: 42, height: 42, borderRadius: 12, border: "none", background: "rgba(255,255,255,0.15)", color: "#ffd86b", fontSize: "1.2rem", cursor: "pointer" }}>⚙️</button>
          </div>
        </div>
        ...
        <div style={{ padding: "0 32px" }}>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
            <div style={{ position: "relative", flex: 1, minWidth: 220 }}>
              <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "#aab", fontSize: "1rem", pointerEvents: "none" }}>🔍</span>
              <input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search by community, focal person, or category..."
                style={{ width: "100%", padding: "10px 14px 10px 36px", borderRadius: 10, border: "none", fontSize: ".88rem", outline: "none", boxSizing: "border-box", boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}
              />
            </div>
            <button onClick={exportCSV}
              style={{ padding: "10px 18px", borderRadius: 10, border: "none", background: "rgba(255,255,255,0.15)", color: "white", fontWeight: 600, fontSize: ".85rem", cursor: "pointer", backdropFilter: "blur(10px)", display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap", transition: "background .2s" }}
          ...
          {/* Record count */}
          <div style={{ color: "rgba(255,255,255,0.75)", fontSize: ".8rem", marginBottom: 10 }}>
            Showing <strong style={{ color: "white" }}>{sorted.length}</strong> of <strong style={{ color: "white" }}>{data.length}</strong> record{data.length !== 1 ? "s" : ""}
            {search && <span> for "<em>{search}</em>"</span>}
          </div>

          {/* Table */}
          <div style={{ borderRadius: 16, overflow: "hidden", boxShadow: "0 4px 20px rgba(0,0,0,0.18)", marginBottom: 32 }}>
            <div ref={tableRef} style={{ width: "100%", overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", background: "#fff", tableLayout: "auto", minWidth: 900 }}>
                <thead>
                  <tr style={{ background: "linear-gradient(135deg,#1e5bb8,#2a6bd4)", color: "#fff" }}>
                    {COLUMNS.map((col) => (
                      <th key={col.key} style={thStyle(col.key)} onClick={() => toggleSort(col.key)} onMouseEnter={() => setHoveredCol(col.key)} onMouseLeave={() => setHoveredCol(null)}>
                        <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                          {col.label}
                          {col.key !== "#" && (
                            <span style={{ opacity: sortCol === col.key ? 1 : 0.35, fontSize: ".65rem" }}>
                ...
                <tbody>
                  {sorted.length === 0 ? (
                    <tr>
                      <td colSpan={COLUMNS.length} style={{ textAlign: "center", padding: "32px 16px", color: "#aaa", fontSize: ".9rem" }}>
                    ...
                    <tr key={i} onMouseEnter={() => setHoveredRow(i)} onMouseLeave={() => setHoveredRow(null)}>
                      {COLUMNS.map((col) => (
                        <td key={col.key} style={tdStyle(col.wrap, i, col.key)}>
                          {col.key === "budget"
                            ? <span style={{ fontWeight: 700, color: "#1e5bb8" }}>{cellValue(row, col.key, i)}</span>
                            : col.key === "category"
                              ? <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 8, background: "#e8f0ff", color: "#1e5bb8", fontSize: ".72rem", fontWeight: 600 }}>{cellValue(row, col.key, i)}</span>
                              : col.key === "#"
                                ? <span style={{ fontWeight: 700, color: "#999", fontSize: ".75rem" }}>{cellValue(row, col.key, i)}</span>
                ...
                {sorted.length > 0 && (
                  <tfoot>
                    <tr style={{ background: "#f0f5ff", borderTop: "2px solid #d0e3ff" }}>
                      {COLUMNS.map((col) => {
                        const numCols = ["budget", "households", "male", "female", "total"];
                        if (col.key === "community") return <td key={col.key} style={{ padding: "10px 12px", fontSize: ".78rem", fontWeight: 700, color: "#1e3a6e" }}>TOTALS</td>;
                        if (numCols.includes(col.key)) {
                          const sum = sorted.reduce((s, r) => s + Number(r[col.key] || 0), 0);
                          return (
                            <td key={col.key} style={{ padding: "10px 12px", fontSize: ".8rem", fontWeight: 700, color: "#1e5bb8", whiteSpace: "nowrap" }}>
                              {col.key === "budget" ? "₱" + sum.toLocaleString() : sum}
                            </td>
                          );
                        }
                        return <td key={col.key} style={{ padding: "10px 12px" }} />;
                      })}
                    </tr>
                  </tfoot>
                )}
